...
<%@ page import="com.bipowernode.crm.settings.domain.DicValue" %>
<%@ page import="com.bipowernode.crm.workbench.domain.Tran" %>
	...
	* */
	function changeStage(stage,i) {
        $.ajax({
            ...
            success:function (data) {
                ...
                * */
                if (data.success){
                    //改变阶段成功后，需要再详细信息页上局部刷新 刷新阶段，可能性，修改人，修改时间
    ...
    function changIcon(stage,index1){
       ...
       /* alert("当前阶段"+currentStage);
        ...
        alert("当前阶段下标"+index);
        alert("当前正常阶段和后面丢失阶段的分界点下标"+point);*/
       //如果当前阶段可能性为0 ，前七个为黑圈，后两个一个红叉、一个黑叉
       if(currentPossibility=="0"){
           ...
           //遍历后两个
           for(var i=poin;i<<%=dvList.size()%>;i++){
               //如果是当前阶段
                if (i==index){
                //红叉
                $("#"+i).removeClass();
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
        ...
        <%
            ...
            //判断当前阶段
            //如果当前阶段可能性为0 前七个一定是黑圈，后两个一个是红×，一个是黑×
            if("0".equals(currentPossibility)){
                for(int i=0;i<dvList.size();i++){
                    //取出每一个遍历的阶段。根据每一个遍历出来的阶段取其可能性
                    DicValue dv= dvList.get(i);
                    String listStage=dv.getValue();
                    String listPossibility=pMap.get(listStage);
                    //如果遍历出来的阶段的可能性为0，说明最后两个，一个是红叉，一个是黑叉
        ...
        %>
         <span  id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
                ...
                ata-toggle="popover" data-placement="bottom"
  ...
  //如果当前阶段的可能性不为0，前7个有可能是绿圈、绿色标记、黑圈，后两个一定是黑叉
     ...
     for(int i=0;i<dvList.size();i++) {
        ...
        -----------
        <%
         //如果遍历出来的可能性不为0，说明是前七个阶段 绿圈、绿色标记、黑圈
         }else{
             //如果是当前阶段
             if(i==index){
                 //绿色标记——
